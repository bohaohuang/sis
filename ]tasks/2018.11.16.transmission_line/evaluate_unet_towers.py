...
from nn import unet, nn_utils, nn_processor
    ...
    def process(self):
        ...
        for file_cnt, (file_name_list) in enumerate(self.file_list):
            ...
            if self.score_results:
                ...
                ersa_utils.save_file(os.path.join(pred_save_dir, '{}_conf.npy'.format(tile_name)), image_pred)
                if self.score_results:
...
file_list_train = cm.load_files(field_name='Colwich,Clyde,Wilmington',
                                field_id=','.join(str(i) for i in range(4, 16)),
                                field_ext='RGB,GT_switch')
file_list_valid = cm.load_files(field_name='Colwich,Clyde,Wilmington', field_id='1,2,3',
                                field_ext='RGB,GT_switch')
chan_mean = cm.meta_data['chan_mean']

nn_utils.tf_warn_level(3)
'''unet.evaluate(file_list_train, patch_size, None, bs, chan_mean, model_dir, gpu, save_result_parent_dir=suffix,
              sfn=32, force_run=True, score_results=True, split_char='.', load_epoch_num=20)'''

estimator = NNEstimatorSegment(
    model, file_list_train, patch_size, None, bs, chan_mean, model_dir, 'default', suffix, name='nn_estimator_segment',
    gpu=gpu, verb=True, load_epoch_num=20, best_model=False, truth_val=1, score_results=True, split_char='.')
