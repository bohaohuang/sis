...
def make_dataset(rgb_files, info_dir, store_dir, tf_dir):
    writer_train = tf.python_io.TFRecordWriter(os.path.join(tf_dir, 'train_all_t.record'))
    writer_valid = tf.python_io.TFRecordWriter(os.path.join(tf_dir, 'valid_all_t.record'))
...
if __name__ == '__main__':
    ...
    csv_files = natsorted([a for a in glob(os.path.join(data_dir, 'raw', '*.csv'))])
