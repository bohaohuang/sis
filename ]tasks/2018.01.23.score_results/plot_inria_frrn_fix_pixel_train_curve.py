...
run_record_dir = os.path.join(task_dir, 'frrn_inria_fixel_pixel')
...
for batch_size, patch_size in zip(batch_sizes, patch_sizes):
    ...
    for runId in range(5):
        file_name = 'run_FRRN_inria_aug_psbs_{}_PS({}, {})_BS{}_EP100_LR0.0001_DS60_DR0.1_SFN32' \
...
plt.xlabel('Epoch')
plt.ylabel('Xent')
plt.title('FRRN on Inria Training Curve')
plt.tight_layout()

plt.savefig(os.path.join(img_dir, 'frrn_inria_fixpixel_train_curve.png'))
