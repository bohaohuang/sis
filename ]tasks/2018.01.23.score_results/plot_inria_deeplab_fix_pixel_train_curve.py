...
for batch_size, patch_size in zip(batch_sizes, patch_sizes):
    ...
    for runId in range(5):
        file_name = 'run_DeeplabV3_res101_inria_aug_psbs_{}_PS({}, {})_BS{}_EP100_LR1e-05_DS40_DR0.1_SFN32' \
...
plt.xlabel('Epoch')
plt.ylabel('Xent')
plt.title('DeepLab on Inria Training Curve')
plt.tight_layout()

plt.savefig(os.path.join(img_dir, 'deeplab_inria_fixpixel_train_curve.png'))
