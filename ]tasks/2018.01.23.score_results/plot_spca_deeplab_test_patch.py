...
for cnt_run, run_repeat in enumerate(runs):
    for cnt_size, size in enumerate(sizes):
        file_name = 'TDeeplabV3_spca_aug_grid_0_PS(321, 321)_BS5_EP100_LR1e-05_DS40_DR0.1_SFN32_{}_{}_spca.npy'.format(
            size, run_repeat
        )
...
np.save(os.path.join(save_dir, 'spca_deeplab_test_patch.npy'), [sizes, iou_record, duration_record])
