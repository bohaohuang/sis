...
PATCH_NAME = 'deeplab_spca_fileList_{}'
...
def read_flag():
    ...
    parser.add_argument('--patch-dir', type=str, default=PATCH_DIR, help='directory that has all the patches')
    parser.add_argument('--patch-name', type=str, default=PATCH_NAME, help='name of the patch')

    flags = parser.parse_args()
    flags.patch_name = flags.patch_name.format(flags.run_id)
    flags.input_size = (flags.input_size, flags.input_size)
    flags.tile_size = (flags.tile_size, flags.tile_size)
    flags.model_name = flags.model_name.format(flags.patch_name)
    return flags
...
def main(flags):
    ...
    file_list_valid = uabCrossValMaker.make_file_list_by_key(idx, file_list, [i for i in range(0, 250)])
