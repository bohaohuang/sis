...
class DeeplabV3VaryCrop(uabMakeNetwork_DeepLabV2.DeeplabV3):
    ...
    def evaluate(self, rgb_list, gt_list, rgb_dir, gt_dir, input_size, tile_size, batch_size, img_mean,
        ...
        for file_name, file_name_truth in zip(rgb_list, gt_list):
            ...
            reader = uabDataReader.ImageLabelReader(gtInds=[0],
                                                    ...
                                                    block_mean=img_mean,
                                                    overlap=self.crop_pixel*2,
                                                    padding=np.array((self.crop_pixel, self.crop_pixel)),
                                                    isTrain=False)
...
for c_cnt, crop in enumerate(crop_list):
    ...
    model = DeeplabV3VaryCrop({'X':X, 'Y':y},
                              trainable=mode,
...
save_file_name = os.path.join(task_dir, 'deeplab_crop_records.npy')
