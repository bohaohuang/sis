...
def make_cmp_plot(rgb, truth, ft, x, y, window_size, city_str):
    ...
    plt.axis('off')
    plt.subplot(122)
    iou = make_subplot(ft_patch, ft, truth_patch, 'FT')
    plt.tight_layout()
...
if __name__ == '__main__':
    ...
    img_save_dir = os.path.join(img_dir, 'pred_patches', ft_dir.split('/')[-3])
    ...
    for f in eval_files:
        ...
        for patch_cnt in range(len(erp_ft)):
            ...
            fig, iou = make_cmp_plot(rgb, truth, ft, x, y, window_size, '{}_X{}_Y{}'.format(f, x, y))
            if iou >= good_th:
                plt.savefig(os.path.join(img_save_dir, 'good_{}_h{}_w{}.png'.format(f, y, x)))
            elif iou <= bad_th:
                plt.savefig(os.path.join(img_save_dir, 'bad_{}_h{}_w{}.png'.format(f, y, x)))
            plt.close(fig)
