...
GROUP_FILE_DIR = r'/media/ei-edl01/user/bh163/tasks/2018.06.01.domain_selection/file_group_austin.npy'
...
def read_flag():
    ...
    parser.add_argument('--group-file-dir', type=str, default=GROUP_FILE_DIR, help='path to group file dir')
...
def main(flags):
    group_alpha = [0.5, 0.2, 0.1, 0.1, 0.1]
    group_files = np.load(flags.group_file_dir)
    ...
    dataReader_train = uabDataReader.ImageLabelReaderGroupSampleControl(
        [3], [0, 1, 2], patchDir, file_list_train, flags.input_size, flags.batch_size,
        group_alpha, group_files, dataAug='flip,rotate', block_mean=np.append([0], img_mean))
