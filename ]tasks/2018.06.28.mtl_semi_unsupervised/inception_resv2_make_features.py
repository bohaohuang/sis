...
incep = keras.applications.inception_resnet_v2.InceptionResNetV2(include_top=True, weights='imagenet')
avg1536 = keras.models.Model(inputs=incep.input, outputs=incep.get_layer('avg_pool').output)
file_name = os.path.join(task_dir, 'incep_inria_{}.csv'.format(model_name))
patch_file_name = os.path.join(task_dir, 'incep_inria_{}.txt'.format(model_name))
with open(file_name, 'w+') as f:
    with open(patch_file_name, 'w+') as f2:
        for file_line in tqdm(files):
            ...
            output1536 = avg1536.predict(img).reshape((-1,)).tolist()
            writer = csv.writer(f, lineterminator='\n')
            writer.writerow(['{}'.format(x) for x in output1536])
            f2.write('{}\n'.format(patch_name))
