...
def make_cmp_plot(rgb, truth, base, mtl, x, y, window_size, city_str):
    ...
    plt.axis('off')
    plt.subplot(132)
    make_subplot(base_patch, base, truth_patch, 'Before')
    plt.subplot(133)
    make_subplot(mtl_patch, mtl, truth_patch, 'After')
    plt.tight_layout()
...
if cnn_name == 'unet':
    ...
    mtl_dir = r'/hdd/Results/ugan/UnetGAN_V3_inria_gan_xregion_0_PS(572, 572)_BS20_' \
...
for city_num in [0, 1, 2, 3, 4]:
    for val_img_cnt in range(1, 6):
        img_save_dir = os.path.join(img_dir, 'uganv3shrink_xregion', city_list[city_num])
        ...
        truth, base, mtl = imageio.imread(truth_img_name), imageio.imread(base_img_name), \
                           imageio.imread(mtl_img_name)
        truth = truth / 255
        base = base / 255

        assert np.all(np.unique(truth) == np.array([0, 1])) and np.all(np.unique(base) == np.array([0, 1])) \
               and np.all(np.unique(mtl) == np.array([0, 1]))
        ...
        for patch_cnt in range(top_patch_check):
            x, y = erp_base[patch_cnt, :][:2]
            fig = make_cmp_plot(rgb, truth, base, mtl, x, y, window_size,
