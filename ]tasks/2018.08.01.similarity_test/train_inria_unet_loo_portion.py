...
def read_flag():
    ...
    parser.add_argument('--GPU', type=int, default=GPU, help="GPU used for computation.")
    ...
    parser.add_argument('--portion', type=int, default=PORTION, help='#tiles from target city kept in training data')

    flags = parser.parse_args()
    flags.input_size = (flags.input_size, flags.input_size)
    flags.tile_size = (flags.tile_size, flags.tile_size)
    flags.model_name = flags.model_name.format(flags.finetune_city, flags.portion, flags.run_id)
    return flags
...
def main(flags):
    ...
    for i in range(5):
        for j in range(1, 37):
            ...
            elif i == flags.finetune_city and j <= flags.portion:
                filter_valid.append(j * 10 + i)
            elif i == flags.finetune_city and j > flags.portion:
                filter_train.append(j * 10 + j)
    # use first city for validation
    ...
    model.train_config('X', 'Y', flags.n_train, flags.n_valid, flags.input_size, uabRepoPaths.modelPath,
                       loss_type='xent', par_dir='Inria_Domain_Selection/Portion')
