...
def make_res50_features(model_name, task_dir, GPU=0, force_run=False):
    ...
    feature_file_name = os.path.join(task_dir, 'res50_{}_{}.csv'.format(target_city, model_name))
    patch_file_name = os.path.join(task_dir, 'res50_{}_{}.txt'.format(target_city, model_name))
    ...
    if model_name == 'deeplab':
        ...
        input_size = (572, 572)
        overlap = 184
    blCol = uab_collectionFunctions.uabCollection(target_city)
...
feature_file_name, patch_file_name, ps, patchDir, idx = mrf(model_name, task_dir, GPU=GPU, force_run=False)
