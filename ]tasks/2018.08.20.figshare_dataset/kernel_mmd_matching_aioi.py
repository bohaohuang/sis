...
feature_file_name, patch_file_name, ps, patchDir, idx = mrf(model_name, task_dir, GPU=GPU, force_run=force_run)
feature = pd.read_csv(feature_file_name, sep=',', header=None).values
with open(patch_file_name, 'r') as f:
    patch_names = f.readlines()
target_feature_file_name, _, _, _ = make_res50_features(model_name, task_dir, city_name, GPU=GPU, force_run=force_run)
target_feature = pd.read_csv(target_feature_file_name, sep=',', header=None).values

# 2. make city and building truth
truth_city = make_city_truth(task_dir, model_name, patch_names, force_run=force_run)
truth_building = make_building_truth(ps, task_dir, model_name, patchDir, patch_names, force_run=force_run)
