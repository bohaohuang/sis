...
def patch_iterator(parent_dir, file_list, patch_size, img_mean):
    for files in file_list:
        ...
        for cnt, f in enumerate(files[:3]):
            img[:, :, cnt] = crop_center(imageio.imread(os.path.join(parent_dir, f)), patch_size[0], patch_size[1])
...
file_name = os.path.join(task_dir, 'vae_inria.csv')
patch_file_name = os.path.join(task_dir, 'vae_inria.txt')
