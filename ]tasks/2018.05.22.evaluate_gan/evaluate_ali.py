...
for lr in ['0.0001', '5e-05', '1e-05']:
    for z_dim in [500, 800, 1000]:
        ...
        model_dir = r'/hdd6/Models/ALI_Inria/ALI_inria_z{}_lrm1_rawm_PS(64, 64)_BS128_EP50_LR{}_DS50_DR0.1_SFN64'\
            .format(z_dim, lr)
        ...
        new_task_dir = os.path.join(task_dir, 'ali')
        if not os.path.exists(new_task_dir):
            os.makedirs(new_task_dir)
        file_name = os.path.join(new_task_dir, 'ali_inria_lr{}_z{}.csv'.format(lr, z_dim))
        patch_file_name = os.path.join(new_task_dir, 'ali_inria_lr{}_z{}.txt'.format(lr, z_dim))
