...
gmm_models = []
model_file_name = os.path.join(task_dir, 'gmm_models_{}.npy'.format(model_name))
...
for i in range(5):
    ...
    for j in range(5):
        llh = gmm_models[j].predict_proba(feature_valid[
                                          np.all([truth_city_valid == i, truth_building_valid == 1], axis=0), :])
        tid = track_id_valid[np.all([truth_city_valid == i, truth_building_valid == 1], axis=0)]
        llh = np.mean(llh, axis=1)
        ...
        plt.title('{} model on {}'.format(city_list[j], city_list[i]))
    plt.tight_layout()
