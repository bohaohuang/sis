...
def make_cmp_plot(rgb, truth, pred, pred_cmp, city_str, rect, building_size):
    ...
    plt.axis('off')
    ax2 = plt.subplot(132)
    make_subplot(pred_patch, 'U-Net')
    ax2.add_patch(rect[1])
    ax3 = plt.subplot(133)
    make_subplot(pred_cmp_patch, 'DeepLab')
...
for city_num in range(4, 5):
    ...
    if force_run:
        for val_img_cnt in range(1, 6):
            ...
            unet_base_img_name = os.path.join(cnn_base_dir, '{}{}.png'.format(city_list[city_num], val_img_cnt))
        ...
        np.save(os.path.join(task_dir, '{}_unet.npy'.format(city_list[city_num])), unet_miss_size)
        np.save(os.path.join(task_dir, '{}_deeplab.npy'.format(city_list[city_num])), deeplab_miss_size)
    else:
        both_miss_size = np.load(os.path.join(task_dir, '{}_both.npy'.format(city_list[city_num])))
        unet_miss_size = np.load(os.path.join(task_dir, '{}_unet.npy'.format(city_list[city_num])))
        deeplab_miss_size = np.load(os.path.join(task_dir, '{}_deeplab.npy'.format(city_list[city_num])))
