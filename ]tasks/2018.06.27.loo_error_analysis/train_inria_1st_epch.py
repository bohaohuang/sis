...
def read_flag():
    ...
    parser.add_argument('--input-size', default=None, type=int, help='input size 224')
    ...
    parser.add_argument('--cnn-name', type=str, default=CNN_NAME, help='cnn network name')

    flags = parser.parse_args()
    if flags.cnn_name == 'unet':
        flags.input_size = (572, 572)
    else:
        flags.input_size = (321, 321)
    flags.tile_size = (flags.tile_size, flags.tile_size)
    flags.model_name = flags.model_name.format(flags.cnn_name, flags.run_id)
    return flags


def main(flags, n_train):
    ...
    # train
    model.train_config('X', 'Y', n_train, flags.n_valid, flags.input_size, uabRepoPaths.modelPath,
    ...
    print('cnn: {}, n_train: {}, duration {:.2f}s'.format(flags.cnn_name, n_train, duration))


if __name__ == '__main__':
    flags = read_flag()
    if flags.cnn_name == 'unet':
        n_train_list = [3690, 7380, 11070, 14760]
    else:
        n_train_list = [5390, 10780, 16170, 21560]
