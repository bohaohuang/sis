...
PRE_TRAINED_MODEL = r'/home/lab/Documents/bohao/code/sis/test/models/UnetInria_fr_mean_reduced/UnetInria_fr_mean_reduced_EP-100_DS-60.0_LR-0.0001'
...
def main(flags):
    ...
    with tf.Session(config=config) as sess:
        ...
        if os.path.exists(PRE_TRAINED_MODEL) and tf.train.get_checkpoint_state(PRE_TRAINED_MODEL):
            latest_check_point = tf.train.latest_checkpoint(PRE_TRAINED_MODEL)
        ...
        try:
            ...
            model.train('X', 'Y', flags.epochs, flags.n_train, flags.batch_size, sess, train_summary_writer,
                        train_reader=reader_train_op, valid_reader=reader_valid_op, image_summary=sis_utils.image_summary)
        finally:
            coord.request_stop()
            coord.join(threads)
